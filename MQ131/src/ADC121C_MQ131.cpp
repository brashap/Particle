...
float ADC121C_MQ131::Measure_Resistance(int rawadc)
{
    ...
    Serial.printf("rawADC %04X, VRL %0.2f, VREF %0.2f, RL %0.2f, RsAir %0.2f \n", rawadc, VRL, VREF, Measure_RL(), RsAir);
    //Serial.printf("Load Resistance: %0.2f \n ",Measure_RL());
    return RsAir;
}
...
float ADC121C_MQ131::Measure_Rs()
{
   ...
   // Serial.printf("Rs Resistance: %0.2f \n ",Rs);
    return Rs;
}
...
{
    Serial.printf("Ro Resistance: %0.2f \n ",Measure_Ro());
